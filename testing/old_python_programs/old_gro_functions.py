...
def terminus_fix(gro_dataframe):
    ...
    subset_maxgro['atom_type'] = subset_maxgro['atom_type'].replace('O2', 'OT2')
    subset_maxgro['atom_type'] = subset_maxgro['atom_type'].replace('O1', 'OT1')
...
complete_df = pd.merge(merged_df, charges,  how='inner', on=['res_name', 'atom_type'])
...
merged_copies = pd.merge(copy_dist, copy_nb,  how='left', on=['name_Ai', 'name_Bj'])
